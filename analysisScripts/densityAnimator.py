...
while infile:
	...
	if i==xyzSize[0]*xyzSize[1]*xyzSize[2]: # end of a timestep
		...
		if currTStep<start or currTStep>finish:
			...
			#Create the colorbar
			CS3 = imshow(renderTOTPOP.T,vmin=0,vmax=100,cmap=myMap,aspect=myAspect,extent=[0,xyzSize[d1],0,xyzSize[d2]])
			if cbFlag:
			...
			if species == 2: # for multispecies
				...
				xlabel(r'$%s$'%labX)
				ylabel(r'$%s$'%labY)
				plt.axis(xmin=0, xmax=xyzSize[d1], ymin =0, ymax=xyzSize[d2])
